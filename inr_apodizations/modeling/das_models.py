...
class DasInrApodMixerBoxcarForced(DasInrApodMixer):
    """Keras model that forces each apodization channel through a boxcar mask.

    The mask is built from an adimensional q feature, typically xrel_over_z,
    and uses a hard boxcar threshold derived from a global f-number.
    """
    ...
    def __init__(
        ...
        feature_chunk_size: int,
        n_apodizations: int | None = None,
        boxcar_f_number: float = 1.0,
        boxcar_q_feature_index: int = 0,
        boxcar_epsilon: float = 1e-8,
        weight_regularization_enabled: bool = False,
        weight_regularization_lambda: float = 1e-3,
        weight_regularization_tau: float = 0.30,
        weight_regularization_epsilon: float = 1e-8,
        ...
        """Initialize the forced-boxcar mixer.

        Args:
            apodization_model: INR model mapping geometry features to multiple weights.
            features_grid: Feature tensor with shape ``(E, Z, X, F)``.
            feature_chunk_size: Number of feature rows processed per forward chunk.
            n_apodizations: Number of apodizations generated by the INR.
            boxcar_f_number: Global f-number used to derive the hard aperture threshold.
            boxcar_q_feature_index: Feature index containing the adimensional q ratio.
            boxcar_epsilon: Numerical tolerance used at the boxcar boundary.
            Remaining args: Same as :class:`DasInrApodMixer`.

        Raises:
            ValueError: If the forced-boxcar parameters are invalid.
        """
        self.boxcar_f_number = float(boxcar_f_number)
        self.boxcar_q_feature_index = int(boxcar_q_feature_index)
        self.boxcar_epsilon = float(boxcar_epsilon)
        if self.boxcar_f_number <= 0.0:
            raise ValueError("boxcar_f_number must be > 0")
        if self.boxcar_epsilon <= 0.0:
            raise ValueError("boxcar_epsilon must be > 0")
        n_features = int(features_grid.shape[-1])
        if not (0 <= self.boxcar_q_feature_index < n_features):
            raise ValueError(
                "boxcar_q_feature_index is out of bounds: "
                f"{self.boxcar_q_feature_index}, n_features={n_features}"
            )

        q_grid = tf.cast(
            tf.abs(features_grid[..., self.boxcar_q_feature_index]),
            tf.float32,
        )
        self._forced_boxcar_q_grid = q_grid
        self._forced_boxcar_threshold = tf.cast(1.0 / (2.0 * self.boxcar_f_number), tf.float32)
        self._last_boxcar_mask: tf.Tensor | None = None
    ...
    @property
    def boxcar_mask(self) -> tf.Tensor:
        """Return the hard boxcar mask with shape ``(E, Z, X, 1)``."""
        mask = tf.cast(
            self._forced_boxcar_q_grid
            <= (self._forced_boxcar_threshold + self.boxcar_epsilon),
            tf.float32,
        )
        return mask[..., tf.newaxis]

    @property
    def boxcar_threshold(self) -> float:
        """Return the hard threshold used by the forced boxcar."""
        return float(self._forced_boxcar_threshold.numpy())
    ...
    def reconstruct_image(
        ...
        """Reconstruct multiple DAS images and apply the forced boxcar mask."""
        weights_grid = self.predict_weights_grid(training=training)
        boxcar_mask = tf.cast(self.boxcar_mask, weights_grid.dtype)
        masked_weights_grid = weights_grid * boxcar_mask
        weighted_delayed = delayed_batch[..., tf.newaxis] * tf.cast(
            masked_weights_grid[tf.newaxis, ...],
            delayed_batch.dtype,
        )
        predicted_complex = tf.reduce_sum(weighted_delayed, axis=1)
        intermediate_images = tf.abs(predicted_complex)
        ...
        self._last_intermediate_images = intermediate_images
        self._last_apodization_grids = masked_weights_grid
        self._last_boxcar_mask = boxcar_mask
        return combined_image, masked_weights_grid
