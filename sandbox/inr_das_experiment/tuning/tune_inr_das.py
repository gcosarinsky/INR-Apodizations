...
# Try to locate CUDA libdevice (`libdevice.*.bc`) and set XLA_FLAGS
# before importing TensorFlow so XLA can find device bitcode files.
def _find_libdevice_dir():
    """Search common CUDA install locations for libdevice files.

    Returns the directory containing `libdevice.*.bc` or `None`.
    """
    env_vars = ("CUDA_PATH", "CUDA_HOME", "CUDA_DIR", "CUDA_ROOT")
    for ev in env_vars:
        p = os.environ.get(ev)
        if p:
            candidate = os.path.join(p, "nvvm", "libdevice")
            if os.path.isdir(candidate):
                # Verify presence of at least one libdevice file
                for f in os.listdir(candidate):
                    if f.startswith("libdevice") and f.endswith(".bc"):
                        return candidate

    # Check standard Windows CUDA installation folder
    ...
    # Fallback: recursive glob search in likely roots
    search_roots = [
...
if _libdevice_dir:
    # Use deterministic assignment to avoid inheriting stale shell values.
    os.environ["XLA_FLAGS"] = f"--xla_gpu_cuda_data_dir={_libdevice_dir}"
    os.environ.pop("TF_XLA_FLAGS", None)
    print(f"XLA_FLAGS set to: {_libdevice_dir}")
else:
    # Disable XLA JIT when libdevice is unavailable to avoid runtime crashes.
    _disable_xla_fallback = True
    os.environ["TF_XLA_FLAGS"] = "--tf_xla_auto_jit=0"
    os.environ.pop("XLA_FLAGS", None)
    print(
        "Warning: libdevice.*.bc not found; enabling safe fallback with XLA disabled."
        " Set XLA_FLAGS manually to re-enable XLA with CUDA libdevice."
    )

print(
    "Effective XLA env: "
    f"XLA_FLAGS={os.environ.get('XLA_FLAGS', '(unset)')} | "
    f"TF_XLA_FLAGS={os.environ.get('TF_XLA_FLAGS', '(unset)')}"
)
...
if _disable_xla_fallback:
    ...
    print("TensorFlow JIT/XLA disabled for this run (libdevice fallback mode).")
...
def build_model(hp):
    ...
    trainer.compile(
        jit_compile=not _disable_xla_fallback,
